...
struct value_operator_adapter : private F {
 ...
 private:
  template <typename E = F, std::enable_if_t<not std::is_base_of_v<value_operator_type_restricted, E>, int> = 0>
  ...
  template <typename E = F, std::enable_if_t<not std::is_base_of_v<value_operator_no_create_node, E>, int> = 0>
  [[nodiscard]] node_ptr const& fix_node(node_ptr const& node) const noexcept {
