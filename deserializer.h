...
struct value_slice_comparator<string_value<V>> {
  static bool compare(Slice s) {
    if (s.isString()) {
      return s.compareString(V, sizeof(V));
    }
...
struct parameter_executor<factory_simple_parameter<N, T, required, default_v>> {
  ...
  static auto unpack(Slice s) {
    ...
    if (!value_slice.isNone()) {
      return value_reader<T>::read(s.get(N)).wrap_error("when reading value of field "s + N);
    }
    ...
    if (required) {
      ...
      return result_type{default_v::value};
    }
  }
};
...
struct parameter_executor<factory_slice_parameter<N, required>> {
  ...
  using result_type = result<value_type, deserialize_error>;
  constexpr static bool has_value = true;

  static auto unpack(Slice s) -> result_type {
    auto value_slice = s.get(N);
    if (!value_slice.isNone()) {
      return result_type{value_slice};
    }
    ...
    if (required) {
      ...
      return result_type{parameter_type::value};
    }
  }
};
...
struct parameter_executor<expected_value<N, V>> {
  ...
  using value_type = typename parameter_type::value_type;
  using result_type = result<unit_type, deserialize_error>;
  constexpr static bool has_value = false;

  static auto unpack(Slice s) -> result_type {
    if (value_slice_comparator<V>::compare(s.get(N))) {
      return result_type{unit_type{}};
    }

    using namespace std::string_literals;

    return result_type{deserialize_error{"value at "s + N + " not as expected"}};
  }
};
...
template <int I, typename P, typename... Ps>
struct parameter_list_executor<I, parameter_list<P, Ps...>> {
  ...
  static auto unpack(T t, Slice s) -> unpack_result {
    ...
    if constexpr (executor::has_value) {
      ...
      if (result) {
        std::get<I>(t) = result.get();
        return parameter_list_executor<I+1, parameter_list<Ps...>>::unpack(t, s);
      }
      return unpack_result{result.error().wrap("during read of "s + std::to_string(I) + "th parameters value")};
    } else {
      auto result = executor::unpack(s);
      if (result) {
        return parameter_list_executor<I, parameter_list<Ps...>>::unpack(t, s);
      }
...
template <int I>
struct parameter_list_executor<I, parameter_list<>> {
  using unpack_result = result<unit_type, deserialize_error>;

  template <typename T>
  static auto unpack(T t, Slice s) -> unpack_result {
    if (s.length() != I) {
      return unpack_result {deserialize_error{"Superfluous field in object."}};
    }
...
struct deserialize_plan_executor<parameter_list<Ps...>> {
  ...
  static auto unpack(arangodb::velocypack::Slice s) -> unpack_result {
    ...
    if (!s.isObject()) {
      return unpack_result{deserialize_error(std::string{"value is not an object"})};
    }

    // forward to the parameter execution
    auto parameter_result = parameter_list_executor<0, parameter_list<Ps...>>::unpack(parameter, s);
    if (parameter_result) {
...
struct field_dependent_executor<R, value_deserializer_pair<V, D>, value_deserializer_pair<Vs, Ds>...> {
  ...
  static auto unpack(arangodb::velocypack::Slice s, arangodb::velocypack::Slice v)
    ...
    if (value_slice_comparator<V>::compare(v)) {
      std::cout << "found a matching value" << std::endl;
      return deserialize_with<D>(s).visit(visitor{
        [](auto const& v) { return unpack_result{R{v}}; },
        [](deserialize_error const& e) { return unpack_result{e.wrap("during dependent parse")};}
      });
    }
...
struct field_dependent_executor<R> {
  ...
  static auto unpack(arangodb::velocypack::Slice s, arangodb::velocypack::Slice v)
  -> unpack_result {
    return unpack_result{deserialize_error{"no known value found"}};
  }
};
...
struct deserialize_plan_executor<field_dependent<N, VSs...>> {
  ...
  static auto unpack(arangodb::velocypack::Slice s) -> unpack_result {
    ...
    return field_dependent_executor<variant_type, VSs...>::unpack(s, value_slice).visit(visitor{
        [](variant_type const& v) { return unpack_result{std::make_tuple(v)}; },
        [](deserialize_error const& e) { return unpack_result{e.wrap("when parsing dependently on "s + N)}; }
    });
...
struct deserialize_plan_executor<field_dependent<N>> {
  static auto unpack(arangodb::velocypack::Slice s) {
     ...
     */
    return result<unit_type, deserialize_error>{deserialize_error{"empty dependent field list"}};
  }
};
...
auto deserialize_with(arangodb::velocypack::Slice slice)
  ...
  static_assert(is_applicable_r<typename D::constructed_type, factory, typename plan_result_tuple<plan>::type>::value,
                "factory is not callable with the plan result");
